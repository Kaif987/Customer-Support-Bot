...
from service.utils import (
    ...
    remove_tool_calls,
    convert_to_interrupt_chat_message
)
...
def _parse_input(user_input: UserInput) -> tuple[dict[str, Any], UUID]:
    ...
    kwargs = {
            ...
            "config": RunnableConfig(
                configurable={"thread_id": thread_id, "model": user_input.model, "passenger_id": "3442 587242"}, run_id=run_id
            ),
        }
...
@router.post("/invoke")
async def invoke(user_input: UserInput, agent_id: str = DEFAULT_AGENT) -> ChatHistory:
    """
    ...
    try:
        ...
        if snapshot.next: 
            try:
                last_message = langchain_to_chat_message(snapshot.values["messages"][-1])
            except Exception as e:
                raise HTTPException(status_code=500, detail="error while parsing interrupt")
            
            # TODO: right now we are only sending interrupts for a single toolcall but there might be multiple toolcalls
            # maybe we dont need to send so much information about the toolcall
            interrupt_chat_message =  convert_to_interrupt_chat_message(last_message)
            output_messages.append(interrupt_chat_message)

        return ChatHistory(messages=output_messages)
    except Exception as e:
...
async def message_generator(
    ...
    async for event in agent.astream_events(**kwargs, version="v2"):
        ...
        if (
            ...
            # we are expecting an array of message so if there is a single message from the resonse
            # we convert it into a array
    ...
    if snapshot.next: 
        ...
        # TODO: right now we are only sending interrupts for a single toolcall but there might be multiple toolcalls
        # maybe we dont need to send so much informationastream_events about the toolcall
        interrupt_chat_message =  convert_to_interrupt_chat_message(ai_message)
        yield f"data: {json.dumps({'type': 'interrupt', 'content': interrupt_chat_message.model_dump()})}\n\n"
    yield "data: [DONE]\n\n"
